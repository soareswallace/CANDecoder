...
module BR_Frame_Maker(input reset, RX, SP, ERROR, F_ITMSS,
...
always @(posedge SP) begin
	...
	case(estado_atual)
		...
		sts3: begin		//IDENTIFIER
			...
			if (cont >= 9'd10)begin
				$display("Porem Vou mudar pro RTR");
				//$display("Contador: %d" , cont);
				estado_atual <= sts4;
				cont <= 9'd0;
			end
		...
		sts_INT:begin			//INTERMISSION
         ...
         EOF <= 7'd0;
			...
			if (RX == 1'b0)begin							//Nesse caso, já estamos no primeiro
				...
				F_OVRLD <= 1'b0;
				$display("\n\n");
				$display("Porém já estou no Primeiro Bit de Overload");
			end
